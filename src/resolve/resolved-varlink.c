...
static int vl_method_resolve_service(Varlink* link, JsonVariant* parameters, VarlinkMethodFlags flags, void* userdata) {
        ...
        r = dns_question_new_service(&question_utf8, p.name, p.type, p.domain, !(p.in_flags & SD_RESOLVED_NO_TXT), false, DNS_TYPE_SRV);
        if (r < 0)
                return r;

        r = dns_question_new_service(&question_idna, p.name, p.type, p.domain, !(p.in_flags & SD_RESOLVED_NO_TXT), true, DNS_TYPE_SRV);
        if (r < 0)
                return r;
...
static int vl_method_start_browse(Varlink* link, JsonVariant* parameters, VarlinkMethodFlags flags, void* userdata) {
        ...
        log_warning("Entered vl_method_start_browse");
        ...
        log_warning("Exiting vl_method_start_browse");
        return 1;
}
...
static int vl_method_stop_Browse(Varlink* link, JsonVariant* parameters, VarlinkMethodFlags flags, void* userdata) {
        ...
        uint64_t token;
        int r;
        log_warning("Entered vl_method_stop_Browse");
        assert(link);
