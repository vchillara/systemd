...
typedef enum BrowseServiceUpdateFlag {
        ...
        _BROWSE_SERVICE_UPDATE_FLAG_MAX,
...
static int mdns_next_query_schedule(sd_event_source *s, uint64_t usec, void *userdata) {
        ...
        if (sb->delay == 0) {
                sb->delay++;
                /* First query is sent wihtout SD_RESOLVED_NO_CACHE to fetch
                 * answers already in cache. Set SD_RESOLVED_NO_CACHE to make
                 * all subsequent queries go to the network. */
